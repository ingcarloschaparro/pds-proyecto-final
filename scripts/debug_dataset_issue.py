#!/usr/bin/env python3
"""
Script para debuggear el problema del dataset y métricas cero en MLflow
"""
...
def analyze_dataset_structure():
    ...
    print("\n🔬 ANÁLISIS DATOS PLS:")
    pls_data = df[df['label'] == 'pls']
    print(f"Registros PLS: {len(pls_data)}")
    ...
    print("\n🔬 ANÁLISIS DATOS NON-PLS:")
    non_pls_data = df[df['label'] == 'non_pls']
    print(f"Registros non-PLS: {len(non_pls_data)}")
...
def identify_problem():
    ...
    print("1. Datos PLS: Solo tienen 'resumen' (texto simplificado)")
    print("2. Datos non-PLS: Solo tienen 'texto_original' (texto complejo)")
    print("3. El clasificador está comparando:")
    print("   - Clase 0 (non-PLS): Textos originales complejos")
    print("   - Clase 1 (PLS): Resúmenes simplificados")
    print("\n❌ ESTO NO TIENE SENTIDO porque estamos comparando:")
...
def create_alternative_strategy(df):
    ...
    print(f"Muestras disponibles - PLS: {len(pls_data)}, non-PLS: {len(non_pls_data)}")
...
def main():
    """Función principal"""
    print("🐛 DEBUGGING DATASET Y MLFLOW ISSUE")
    print("=" * 60)
    ...
    print("2. Actualizar el script de entrenamiento")
