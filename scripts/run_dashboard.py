...
def check_files():
    ...
    if missing_files:
        ...
        print("\nRecomendaciones:")
        if "src/dashboard/app.py" in missing_files:
            print("- Ejecutar: python scripts/setup_dashboard.py")
        if "data/processed/dataset_clean_v1.csv" in missing_files:
            print("- Ejecutar: dvc repro make_dataset")
...
def start_mlflow():
    """Iniciar MLflow UI en background si no está corriendo"""
    try:
        # Verificar si MLflow está corriendo
        response = requests.get("http://localhost:5000", timeout=2)
        if response.status_code == 200:
            print("MLflow UI ya está ejecutándose en http://localhost:5000")
            return True
    except:
        pass
    ...
    try:
        ...
        if response.status_code == 200:
            print("MLflow UI iniciado exitosamente")
            print("Accede en: http://localhost:5000")
            return True
        else:
            print("Error iniciando MLflow UI")
...
def start_dashboard(port=8501, browser=True):
    """Iniciar el dashboard de Streamlit"""
    dashboard_path = Path("src/dashboard/app.py")
    if not dashboard_path.exists():
        print(f"Archivo del dashboard no encontrado: {dashboard_path}")
        return False

    print("Iniciando Dashboard por favor...")
...
def show_help():
    ...
    help_text = """
Dashboard por favor - Script de Inicio Rápido

USO:
    python scripts/run_dashboard.py [opciones]

OPCIONES:
    -p, --port PORT      Puerto para el dashboard (default: 8501)
    -b, --browser        Abrir navegador automáticamente (default: True)
...
SERVICIOS INICIADOS:
    - Dashboard por favor: http://localhost:{port}
    - MLflow UI: http://localhost:5000

ARCHIVOS NECESARIOS:
    - src/dashboard/app.py (dashboard principal)
    - data/processed/dataset_clean_v1.csv (datos)
    - mlruns/ (experimentos MLflow)

DEPENDENCIAS:
    - streamlit, plotly, pandas, numpy
    - mlflow (opcional para experimentos)
"""
    print(help_text)
...
def main():
    ...
    print("\nINICIANDO DASHBOARD por favor")
    print(f"Puerto: {args.port}")
    print(f"Navegador: {'Sí' if open_browser else 'No'}")
    print(f"MLflow: http://localhost:5000\n")

    # Iniciar dashboard
    success = start_dashboard(port=args.port, browser=open_browser)
    if success:
        print("\nDashboard finalizado correctamente")
    else:
