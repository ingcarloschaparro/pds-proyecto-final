"""
Clasificador para distinguir entre textos PLS y non-PLS
Implementa baseline TF-IDF + Logistic Regression y modelo DistilBERT
"""
import pandas as pd
import numpy as np
...
def preparar_datos_clasificacion(ruta_datos: str) -> Tuple[pd.Series, pd.Series]:
    """
    Prepara datos para clasificación PLS vs non-PLS
...
def entrenar_baseline_tfidf(X_train: pd.Series, y_train: pd.Series,
                           X_test: pd.Series, y_test: pd.Series) -> Dict[str, Any]:
    """
    Entrena modelo baseline TF-IDF + Logistic Regression
...
def guardar_modelo(resultados: Dict[str, Any], ruta_modelo: str) -> None:
    """
    Guarda el modelo y vectorizer entrenados
...
def main():
    ...
    print("Usando dataset completo para asegurar datos PLS válidos...")
