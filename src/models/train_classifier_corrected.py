"""
Clasificador corregido para distinguir entre textos PLS y non-PLS
Usa el dataset alternativo balanceado
"""
import pandas as pd
import numpy as np
...
def preparar_datos_clasificacion_corregida(ruta_datos: str) -> Tuple[pd.Series, pd.Series]:
    """
    Prepara datos para clasificación PLS vs non-PLS usando dataset corregido

    Args:
        ruta_datos: Ruta al archivo CSV con datos corregidos
...
def entrenar_baseline_tfidf_corregido(X_train: pd.Series, y_train: pd.Series,
    ...
    """
    Entrena modelo baseline TF-IDF + Logistic Regression con datos corregidos
    ...
    Returns:
        Diccionario con modelo, métricas y vectorizer
    """
    ...
    print("\nReporte de clasificación:")
    print(classification_report(y_test, y_pred, target_names=['non-PLS', 'PLS']))
...
def log_to_mlflow(resultados: Dict[str, Any], experiment_name: str = "pls_classification_corrected"):
    ...
    with mlflow.start_run(run_name=f"tfidf_corrected_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
        ...
        plt.title('Matriz de Confusión - Clasificador PLS Corregido')
...
def guardar_modelo_corregido(resultados: Dict[str, Any], ruta_modelo: str) -> None:
    """
    Guarda el modelo y vectorizer entrenados (versión corregida)
...
def main():
    ...
    try:
        ...
        print(f"F1 PLS: {resultados['f1_pls']:.4f}")
        print(f"F1 non-PLS: {resultados['f1_non_pls']:.4f}")
